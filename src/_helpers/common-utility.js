...
export const menuPermission = (navigation) => {
    ...
    if (user.user_group_id === '60227751e2e5152364d34551') {
        ...
        for (var keyData in navigation) {
            if (navigation[keyData].module_name !== 'system_modules') {
                if (
                    user.user_permission[navigation[keyData].module_name] !== undefined ||
                    navigation[keyData].module_name === 'dashboard' ||
                    navigation[keyData].module_name === undefined
                ) {
                    permission_nav.push(navigation[keyData]);
                }
            }
        }
        return permission_nav;
